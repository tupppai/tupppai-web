...
class MY_Session extends CI_Session{
    ...
    public function __construct($params = array()) {
        ...
        if ($this->sess_use_redis == TRUE) {
            ...
            $this->redis->setOption(Redis::OPT_PREFIX, 'laravel:'); 
        }
        ...
        $this->sess_read();
        /* jq: we don't create or update session in ci because of laravel
        if ( ! $this->sess_read()) {
            $this->sess_create();
        } else {
            $this->sess_update();
    ...
    function sess_read() {
        ...
        if ($this->sess_use_redis=== TRUE) {
            ...
            //todo: 如果需要加密
            $row = $this->redis->get($session['session_id']);
            $row = unserialize(unserialize($row));
            ...
            if (isset($row['last_activity']) AND $row['last_activity'] != '') {
    ...
    function sess_create() {
        ...
        #todo: 把ci的session和laravel的session生成规则对齐
        $this->userdata = array(
