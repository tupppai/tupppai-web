<?php
class Cache {
    const TIMEOUT           = 2/*second*/;//链接超时

    const DB_PHP_SESSION    = 11;//该DB为session服务专用，切勿使用
    const DB_LOGON          = 1;    //后台登陆的心跳包
    const DB_PROCESS        = 2;    //持有，解决并发（下面的CACHE类型）
    const DB_LOGGER         = 3;    //日志类型
    #const DB_PAY           = 4;    //支付
     ...
     * @input $key 搜索关键字
     */
    public function fetch ($session_id, $key, $online_count) {
        if (!isset($this->r)) {
            return array();
        }
     ...
     * 初始化平台信息
     */
    public function set_log($platform, $type) {
    ...
    public function log ($str){
        ...
        $this->r->lpush($this->platform."_".$this->type, $str);
    }
}
