...
class UmengController extends ControllerBase{
    ...
    public function testAction() {
        $type = $this->get('type', 'int', Message::TYPE_REPLY);
        $count = $this->get('count', 'int', 1);
        
        $token = UserDevice::getUsersDeviceToken(220);
        $text  = Message::getPushMessage($type);
        $custom = array(
            'type'=>$type,
            'count'=>$count
        );
        $this->push($text, $custom, $token);
        pr("推送成功,uid:220,type:$type,count:$count");
    }
