 ...
 * @return string
 */
function jsonFormat($ret = 0, $code = 0, $data = array(), $info = '') {
	#todo: info i18n
...
function error($codeName = 0, $info = '', $data = array()) {
	...
	//todo log error info
	$ret = jsonFormat(0, $code, $data, $info);
	$str = json_encode($ret);
...
function expire($info = '', $data = array()) {
	...
	$ret = jsonFormat(2, $code, $data, $info);
	$str = json_encode($ret);
...
function encodeLocation($province, $city, $location) {
 ...
 */
function matchPhoneFormat($phone) {
	if (strlen($phone) == 11) {
...
function matchUsernameFormat($username) {
 ...
 * @author brandwang
 */
function emojiToShortname($content) {
 ...
 * @param [string] $content
 */
function shortnameToUnicode($content) {
 ...
 * @param [string] $money
 * */
function moneyConvert($money, $type = '', $locale = 'zh_CN') {
 ...
 * http_post请求
 */
function httpPost($url, $post = '', $cookie = '', $returnCookie = 0) {
